...
<div id="centered">
<div id="margin">
    <img class="opcyberguy" src="<?php bloginfo('template_directory');?>/images/opcyberpaint-guy.png" alt="paintball" />
...
<ul id="single">
    <li id="post-<?php the_ID(); ?>" <?php post_class('content'); ?>>
    	<h2><?php if ( is_page()) : else : ?><span><?php the_time('M') ?><br/><?php the_time('d') ?></span><?php endif; ?><?php if ( get_the_title() ) { the_title();} else { echo '(No Title)';} ?></h2>
		<?php the_content(); ?>
...
<ul id="comments">
    <li class="sponsors">sponsored by: </li>
    <li class="stars"></li>
</ul>
...
<div id="footer">
<p>Original template: <a href="http://schwarttzy.com/about-2/" target="_blank">Semper Fi</a> | edited by <a href="http://jeltelagendijk.nl" target="_blank">Jelte Lagendijk</a> | Powered by Wordpress <!-- <?php echo get_num_queries(); ?> queries in <?php timer_stop(1); ?> seconds --></p>
</div>
...
<script type="text/javascript">
    ...
    if ( has_post_thumbnail() ) { // check if the post has a Post Thumbnail assigned to it.
        $x = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full');
        $image_url = $x[0];?>
        $.backstretch("<?php print($image_url) ?>");
    <?php    
    } else {?>
        $.backstretch("<?php bloginfo('template_directory');?>/images/might.jpg");
    <?php } ?>
